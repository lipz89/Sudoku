...
namespace Sudoku2
{
    public partial class FrmGame : Form
    {
        ...
        protected override void WndProc(ref Message m)
        {
            if (m.Msg == MutexOnly.WM_COPYDATA)
            {
                ...
                if (dwData == MutexOnly.WM_OPENNEWFILE)
                {
                    ...
                    if (!string.IsNullOrEmpty(fileName) && fileName.EndsWith(".sdk"))
                    {
                        RunAndLoad(fileName);
                    }
                }
        ...
        {
            mGame.ShowClew = tsmiClew.Checked;
        }
        ...
        private void tsmiEject_Click(object sender, EventArgs e)
        {
            mGame.Eject = tsmiEject.Checked;
            tsmiLimit.Enabled = tsmiEject.Checked;
            if (!tsmiEject.Checked)
            {
                tsmiLimit.Checked = false;
                mGame.Limit = false;
            }
        }

        private void tsmiAboutSudoku_Click(object sender, EventArgs e)
        {
